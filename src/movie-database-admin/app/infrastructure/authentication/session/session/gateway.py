...
class SessionGateway:
    ...
    async def get_session(self, session_id: UUID) -> Session:
        ...
        data = await self.connection.hgetall(
            name=f"auth_sessions:session_id:{session_id}"
        )
        if not data:
    ...
    async def _delete_session(self, superuser_id: UUID) -> None:
        session_id = await self.connection.get(
            name=f"auth_sessions:superuser_id:{superuser_id.hex}"
        )
